...
class Queen {
    ...
    isValid = async (board, r, col, n) => {
        ...
        // currentColumn.style.backgroundColor = "#ff1cff";
        await q.delay();
        ...
        for (let i = r - 1; i >= 0; --i) {
            ...
            if (value == queen) {
                column.style.backgroundColor = "#81fb07";
                currentColumn.innerHTML = "-"
                return false;
            }
            column.style.backgroundColor = "#ca3aff";
            await q.delay();
        }
        ...
        for (let i = r - 1, j = col - 1; i >= 0 && j >= 0; --i, --j) {
            ...
            if (value == queen) {
                column.style.backgroundColor = "#e707fb";
                currentColumn.innerHTML = "-"
                return false;
            }
            column.style.backgroundColor = "#3ad4ff";
            await q.delay();
        }
        ...
        for (let i = r - 1, j = col + 1; i >= 0 && j < n; --i, ++j) {
            ...
            if (value == queen) {
                column.style.backgroundColor = "#d207fb";
                currentColumn.innerHTML = "-"
                return false;
            }
            column.style.backgroundColor = "#adff3a";
            await q.delay();
        }
        return true;
    }
...
playButton.onclick = async function visualise() {
    ...
    if (n > 8) {
        numberbox.value = "";
        alert("Queen value should be between 1 and 8");
        return;
    } else if (n < 1) {
        numberbox.value = "";
        alert("Please Enter a Valid Queen Value");
        return;
    }
    ...
    for (let k = 0; k < array[n]; ++k) {
        ...
        for (let i = 0; i < n; ++i) {
            ...
            for (let j = 0; j < n; ++j) {
                ...
                (i + j) & 1
                    ? (col.style.backgroundColor = "#bb1cff")
                    : (col.style.backgroundColor = "#fc90ca");
                col.innerHTML = "-";
                col.style.border = "0.3px solid #c19e2c";
            }
        }
